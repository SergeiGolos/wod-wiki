...
**Seed**:
The immutable, timestamped compilation of the bundled content corpora (Canvas pages, syntax guides, Collections/Feeds, Efforts, Dashboard Notes, the page template) into versioned artifacts a static host serves and the app imports into Storage (`wodwiki-db`; import seam — `docs/prototypes/seed-data-unification.md`). The Seed is the only path bundled content takes into Storage. The compiler (`scripts/generate-seed.ts`, run by the playground build) emits `public/seed/`: a manifest + content-hashed chunks of `{ path, content }` notes rows and `block-index.<n>` chunks of precomputed `BlockIndexRow[]` corpus rows. Seed provenance (`'seed' | 'user'`) decides whether the Seed Import may overwrite or delete a row — user edits always win. After the import, **every runtime content read loads from Storage**: canvas routes, collections/feeds, dashboards, guide snippets, and view-source go through the seed-content seam (`src/services/content/seedContent.ts`), and the Query Service reads one `block_index` store (user + corpus rows; the generated `static-block-index.json` predecessor is retired).
