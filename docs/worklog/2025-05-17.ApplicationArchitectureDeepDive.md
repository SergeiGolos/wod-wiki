...
- **`AbstractBlockLifecycle.ts`**: A crucial base class implementing the Template Method pattern. It defines a standardized lifecycle for blocks:
  - `enter()`: Called when a block becomes active.
  - `next()`: Called to advance the block's internal state or move to a subsequent element.
  - `leave()`: Called when a block completes or is exited.
  - These template methods handle common logic (like logging, as per MEMORY[9983c68b-a92b-4d98-a500-6541c383c830]) and call abstract hook methods (`doEnter`, `doNext`, `doLeave`) for specific block implementations. (MEMORY[b97f46e6-e823-47fc-aa68-c26c7375316b])
...
- Implementations:
  ...
  - `GroupRepeatingStrategy`: For `RepeatingBlock`s (e.g., standard rounds).

#### 2.3.4. Runtime Actions (`src/core/runtime/actions/`)

Represent operations that modify the runtime state or trigger side effects (like UI updates or sound playback). They are typically generated by the `doEnter`, `doNext`, or `doLeave` methods of `IRuntimeBlock`s.
...
- **Nodes**:
  ...
  - `StatementNode`: Represents a line or segment from the parsed script, containing an array of `Fragment`s and child `StatementNode`s.
  - `PrecompiledNode`: A processed version of `StatementNode`, likely used by the runtime. It provides helper methods like `duration()`, `effort()`, `rounds()`, etc., which use the reader functions.
...
- **Group Operations** (MEMORY[eddef35c-1130-4575-b2a2-0339144edb59]):
  - The system supports different ways child elements in a group are processed over rounds (Round-Robin, Compose, Repeat), likely annotated on `StatementNode`s or `PrecompiledNode`s.
...
- **Generation**:
  ...
  - `AbstractBlockLifecycle` initializes `MetricsContext` for each block, often using a `MetricsFactory`. (MEMORY[a377972a-fc4e-4d35-99ec-0f91d2e63473])
...
- **`ResultSpan`**:
  - Each executed block instance generates a `ResultSpan` (managed via `BlockContext` and `AbstractBlockLifecycle`).
  - Concrete block types override `AbstractBlockLifecycle.enhanceResultSpan()` to populate the `ResultSpan` with their specific metrics (e.g., `EffortBlock` inherits, `RepeatingBlock` multiplies, `TimedGroupBlock` and `RootBlock` add/aggregate).
...
- **Template Method Pattern**: Used in `AbstractBlockLifecycle` to define a common execution skeleton for all `IRuntimeBlock`s.
...
- `src/core/`
  ...
  - `parser/` (contains `timer.parser.ts`, `timer.visitor.ts` or similar logic)
  - `lexer/` (contains `timer.tokens.ts` or similar logic)
  - `runtime/`
    ...
    - `blocks/`:
      - `AbstractBlockLifecycle.ts`, `BlockContext.ts`
      - Concrete block implementations (e.g., `EffortBlock.ts`, `RootBlock.ts`).
