...
2. **Cache equivalence:** `QueryCacheKey` = normalized AST (via `serialize.ts` canonical form), resolved range instants, effective timezone, captured evaluation instant, store `generationId`, active system default units (13) — plus temporal kind/bucket semantics and persisted normalization intent where they affect results (12/17). Cache validity additionally requires that the producing scan covered the complete metric-date selection for every metric in the query (the V17 `by-metricDate` completeness guarantee from ticket 14) — resolved range instants alone do not prove completeness. Cached values keep full precision and lineage (never rounded chart values); formula results track input dependencies so an input's regeneration invalidates dependents.
3. **Invalidation signals:** commits to `results`, `notes`, `efforts`, and the catalog (14's atomic mutation paths) increment an in-memory `generationId` and post `ANALYTICS_MUTATION` on `BroadcastChannel('wodwiki.analytics')`; other tabs drop affected cache entries; catalog commits also invalidate typeahead providers (15's notification seam). A received mutation receipt advances the consuming tab's local epoch so generation comparisons stay correct across reloads and equal-counter states ([query freshness deepening](../deepening/06-query-freshness.md) §6).
4. **Races:** each execution samples `generationId` at ticket 19's post-prerequisite checkpoint and at completion — "start" means after the rollup-ensure commit (19 scope 7), never before the asynchronous warmup; publication requires both latest-in-scope run ordering and an unchanged generation, so an older request finishing after a newer one is discarded even at an unchanged generation; a result computed across a mutation is discarded, never written over fresher data; user-visible refresh states distinguish stale-then-updated from unchanged.
