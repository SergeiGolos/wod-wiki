...
Each tier document below includes, per item: what exists today (with file:line references), why it's a problem, and a side-by-side sketch of the old vs. new code shape. Tier 1 and Tier 2's sketches now describe what was actually implemented (see their verification appendices); Tier 3 remains a proposed design derived from the review — nothing in it has been implemented yet.
