...
| 3 | [Tier 3 — Extensibility Realignment](./architectural-cleanup-tier-3-extensibility.md) | ⚠️ **Apparently implemented, not verified** | Why "add a dialect" / "program a behavior for an effort" / "dialect-specific analytics" are harder than they should be today, and how to fix it using mechanisms that already exist | Medium — changes the public extension surface |
...
4. ⚠️ **Tier 3, item 13** (registries + exports) — the prerequisite for everything else in Tier 3; converts three hardcoded lists into one registration pattern that's actually exported to consumers. **Apparently done** — a `Registry<T>` class, `dialectRegistry`, and `strategyRegistry` all now exist and are exported from `core-entry.ts`, closely matching this item's sketch — but this has not been through the same independent-audit rigor as Tiers 1–2. See the per-item notes in the Tier 3 doc.
5. ⚠️ **Tier 3, items 11–12** (hint contract, effort hints) — delivers the "program behavior against an effort" and "dialect analytics" goals on top of a mechanism (`hints`) that already exists and is already understood by strategies. **Apparently done** — `CONSUMED_HINTS`, `IEffort.hints`, and the `EffortEnrichmentPass` child-recursion fix are all present. Not independently verified.

Each tier document below includes, per item: what exists today (with file:line references), why it's a problem, and a side-by-side sketch of the old vs. new code shape. Tier 1 and Tier 2's sketches now describe what was actually implemented, and were checked with the same rigor: independent code audits, real bugs found and fixed, full test/build re-verification (see their verification appendices). **Tier 3's implementation appeared mid-cleanup, from a source outside this review, and has only been lightly inspected — not audited.** Treat its "done" status as provisional until someone runs the same verification pass Tiers 1 and 2 got.
