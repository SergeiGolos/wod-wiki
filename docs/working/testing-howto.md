...
- **[testing-surface-plan.md](testing-surface-plan.md)** — implementation plan for
  consolidating six overlapping harness modules into two seams (TestScript +
  BehaviorTestHarness), closing the parser-to-runtime seam gap, deleting dead
  modules, and reducing unit-setup tax. Six phases, ordered by dependency.
- **[assertions-dsl-deepening.md](assertions-dsl-deepening.md)** — detailed analysis
  of the assertions DSL's current shallowness and a proposed deepened interface
  with domain-specific query methods (roundState, timerState, hasMetric, etc.)
  that eliminate the copy-pasted helper functions in compliance tests.
- **[runtime-testability-plan.md](runtime-testability-plan.md)** — the original
  Stories 1–5 plan that produced the current TestScript + FakeRpcTransport
  infrastructure. Mostly landed; see test-inventory.md for remaining work.
- **[test-inventory.md](test-inventory.md)** — classification of all 86 test files
  (KEEP-AS-BROWSER, KEEP-AS-REACT, REWRITE-ON-BUILDER, ALREADY-PORTED, OUT-OF-SCOPE).