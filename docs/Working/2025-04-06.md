...
*   [x]: 00:06 - Enhanced `WodTable.tsx` to group exercises by effort type with summary rows showing totals.
*   [x]: 00:09 - Enhanced `WodResults.tsx` to also group by exercise effort instead of block ID.

### Next Steps

*   [x]: 00:00 - All tasks completed.

## 00:20 - Plan Add Collapsible Sections to Results Tables
1. Add state management for tracking which exercise groups are expanded/collapsed
2. Update `WodResults.tsx` to implement collapsible sections with toggle functionality
3. Update `WodTable.tsx` to implement the same collapsible functionality
4. Add visual indicators (arrows or icons) to show collapsed/expanded state

### Summary

We'll enhance the workout results tables by making the exercise group sections collapsible. This will allow users to focus on specific exercises by expanding only the relevant sections while keeping others collapsed. The implementation will use React state to track which sections are expanded, and add click handlers to the group headers to toggle their state.

### Completed Steps

* [x]: 00:20 - Plan created.
* [x]: 00:21 - Updated `WodResults.tsx` with collapsible sections.
* [x]: 00:21 - Updated `WodTable.tsx` with collapsible sections.

### Next Steps

* [x]: 00:21 - All tasks completed.
