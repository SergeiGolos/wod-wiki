...
* [ ] 22:22 - Consider applying similar collapsible pattern to other debug panels in the application
...
* [ ] 23:32 - Add `isLeaf` property to `StatementNode` interface in `timer.types.ts`
* [ ] 23:32 - Update `wodBlock` method in `MdTimerInterpreter` to set `isLeaf` based on lap fragments
* [ ] 23:32 - Modify `gotoBlock` method in `TimerRuntime` to check for the explicit `isLeaf` property
* [ ] 23:32 - Update `RuntimeJit.compile` to handle nodes that are leaves but have children
* [ ] 23:32 - Write tests for the new lap-based workout structure
* [ ] 23:32 - Update documentation with usage examples
