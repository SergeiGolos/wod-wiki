...
This document outlines the design for extending WodWiki's fragment system to support **collectible fragments** — fragments that are defined in the workout syntax but have no predetermined value. These fragments signal that data needs to be collected either:

1. **During runtime execution** — Values generated automatically (e.g., elapsed time)
2. **Post-workout in analytics** — Values entered manually by the user
...
- `10` reps → RepFragment with value=10
- `5:00` timer → TimerFragment with value=300000ms
- `100m` distance → DistanceFragment with value={amount: 100, units: 'm'}
...
- All changes are **additive** — existing workouts continue to work
- `collectionState` defaults to `Defined` for existing fragments
- Parser changes only affect workouts using `?` placeholder

### Performance

- Collection state is a simple enum — minimal memory overhead
- Runtime-generated values are collected at block completion — no polling
...
| Collectible fragment never collected | Display "—" in analytics |
...
1. ✅ Parser accepts `?` placeholder in place of numbers
2. ✅ Fragments correctly identify collection state
3. ✅ Runtime generates elapsed-time values automatically
4. ✅ Analytics view displays collectible fragments
5. ✅ Users can edit collected values post-workout
