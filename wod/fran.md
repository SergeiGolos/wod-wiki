# Fran

**Category**: CrossFit Benchmark  
**Type**: For Time  
**Difficulty**: Advanced  

## Description
One of the most famous CrossFit benchmark workouts. Fran is a sprint that tests both metabolic conditioning and strength endurance.

## WOD Wiki Syntax
```
(21-15-9) 
  Thrusters 95lb
  Pullups
```

## Breakdown
- **Rounds**: 3 rounds with descending rep scheme (21-15-9)
- **Exercises**: 
  - Thrusters at 95lb (barbell thrusters)
  - Pullups
- **Goal**: Complete as fast as possible

## Target Times
- **Elite**: Under 2 minutes
- **Advanced**: 2-4 minutes  
- **Intermediate**: 4-6 minutes
- **Beginner**: 6+ minutes

## Tips
- Break thrusters early to avoid failure
- Use kipping pullups if allowed
- Focus on fast transitions between exercises

## Scaling Options
- **Beginner**: Reduce weight to 65lb, use band-assisted pullups
- **Intermediate**: Use 75lb, strict pullups
- **Advanced**: Rx'd as written, consider 115lb for elite athletes
