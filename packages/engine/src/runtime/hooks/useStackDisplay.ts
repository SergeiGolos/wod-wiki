 ...
 *
 * Engine-owned display shape (moved out of the app's MetricSourceRow during
 * the #970 cutover); StackFragmentEntry extends this with an IRuntimeBlock.
 */
export interface FragmentSourceEntry {
    ...
    label?: string;
    /** Explicit status; derived from isLeaf when unset */
    status?: 'pending' | 'active' | 'completed' | 'failed' | 'skipped';
    /** Duration in milliseconds (for duration column) */
    duration?: number;
    /** Grouped metric containers (multi-display blocks) */
