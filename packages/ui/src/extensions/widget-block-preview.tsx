...
/** Minimal react-dom root surface used by the widget DOM bridge. */
type Root = { render: (c: React.ReactNode) => void; unmount: () => void };

// ── Widget authoring contract (mirrors src/components/Editor/widgets/types) ──
...
// ── Local atoms (ported from src/components/atoms — ui-package-local copies
//    because the package cannot import the @/ alias) ────────────────────────

export type WidgetEditButtonMode = "view" | "editing" | "error";

const WIDGET_EDIT_BUTTON_LABELS: Record<WidgetEditButtonMode, string> = {
  view: "Edit widget",
  editing: "Save widget",
  error: "Undo changes",
};
...
const WidgetEditButton = React.forwardRef<
  HTMLButtonElement,
...
const EditableMarkdown = React.forwardRef<
  HTMLTextAreaElement,
  Omit<React.TextareaHTMLAttributes<HTMLTextAreaElement>, "children"> & { hasError?: boolean }
...
function WidgetBlockPreviewWrapper({
  ...
  const previewComponent = registry.get(widgetName);
...
export function widgetBlockPreview(registry: WidgetRegistry): Extension {
  const widgetPreviewField = StateField.define<DecorationSet>({
    ...
    update(value, tr) {
      ...
      // 3. The selection moved (cursor inside a widget reveals its source)
