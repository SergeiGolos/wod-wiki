 ...
 * - **Time / Spans** = raw clock recordings — this metrics (source of truth)
...
export class SpansMetric implements IMetric {
   ...
   */
  constructor(
    readonly spans: readonly TimeSpan[] | TimeSpan[],
    readonly sourceBlockKey?: string,
    readonly timestamp?: Date,
  ) {}

  /** The raw TimeSpan array */
  get value(): readonly TimeSpan[] | TimeSpan[] {
    return this.spans;
  }
  ...
  get image(): string {
    ...
    if (this.spans.length === 1) {
      ...
      if (span.started === span.ended) {
        return SpansMetric.formatTimestamp(span.started);
      }
      return `${SpansMetric.formatTimestamp(span.started)} – ${SpansMetric.formatTimestamp(span.ended ?? Date.now())}`;
    }
