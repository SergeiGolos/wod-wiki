...
/** Creates an INowProvider that reads the browser sender-clock when available. */
export function createBrowserRuntimeNow(): INowProvider {
  return {
    now: () => new Date(getRuntimeNowMs()),
    nowMs: () => getRuntimeNowMs(),
  };
}
