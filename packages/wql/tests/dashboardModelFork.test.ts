...
function md(type: string, content: string): DashboardSectionInput {
  return { type: 'markdown', subtype: type, content };
}

function query(
  body: string,
  opts: string | { widgetType?: string; spanCols?: number; spanFull?: boolean } = {},
): DashboardSectionInput {
  const spec = typeof opts === 'string'
    ? parseQueryWidgetSuffix(opts)
    : {
  ...
  return {
    ...
    widgetError: 'error' in spec ? spec.error : undefined,
  };
}
...
describe('buildDashboardDocument', () => {
  ...
  it('keeps the full document body verbatim; no positional split (decision 22)', () => {
    const doc = buildDashboardDocument([query('max:calc.e1rm{}')], {});
    expect(doc.widgets[0].body).toBe('max:calc.e1rm{}');
    expect(doc.widgets[0].attributes).toEqual({});
    const multi = buildDashboardDocument(
      [query('sum:distance{}\nshow in km') as never],
      {},
    );
    expect(multi.widgets[0]!.body).toBe('sum:distance{}\nshow in km');
  });
