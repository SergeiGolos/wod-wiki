...
import { CodeStatement, ParsedCodeStatement } from '../src/models/CodeStatement';
import { IMetric, MetricType, MetricOrigin } from '../src/models/Metric';
import { IMetricSource } from '../src/contracts/IMetricSource';
import { CodeMetadata } from '../src/models/CodeMetadata';
...
class TestCodeStatement extends CodeStatement {
  ...
  parent?: number;
  children: number[][] = [];
  meta: CodeMetadata = new CodeMetadata(1, 0, 10, 10, 1, 10, 'test');
  ...
  constructor(id: number, metrics: IMetric[]) {
    super();
    this.id = id;
    this.metrics = new MetricContainer(metrics, id);
  }
}
...
describe('CodeStatement implements IMetricSource', () => {
  ...
  describe('getDisplayMetrics', () => {
    ...
    it('should apply type filter', () => {
      ...
      const stmt = new TestCodeStatement(1, metrics);
      const result = stmt.getDisplayMetrics({
        types: [MetricType.Duration, MetricType.Action],
      });
      expect(result).toHaveLength(2);
