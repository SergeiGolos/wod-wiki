 ...
 * and promotion behaviors.
 */

import { describe, expect, it } from 'vitest';
import type { IMetric } from '../src/models/Metric';
import { MetricType } from '../src/models/Metric';
import { createMetricOwnershipLedger } from '../src/ownership';
...
describe('explain() coverage', () => {
  ...
  it('explain() accurately exposes all four outcome types in a single type bucket', () => {
    ...
    expect(explanation.suppressed).toBe(true);
    const suppressors = explanation.entries.filter((e) => e.outcome === 'suppressor');
    const hiddenBySuppressor = explanation.entries.filter(
      (e) => e.outcome === 'hidden-by-suppressor',
    );
    expect(suppressors).toHaveLength(1);
...
describe('suppress/hide semantics', () => {
  it('any suppressor at any layer suppresses the entire type (layer-agnostic suppress semantics)', () => {
    ...
    expect(ledger.raw({ types: [MetricType.Action] })).toEqual([
      parserSuppressor,
      dialectAction,
    ]);
  });
...
describe('promotionCandidates() behaviors', () => {
  ...
  it('candidates for a suppressed type point at the blocking suppressor layer', () => {
    ...
    const runtimeSuppressor = metric({
      type: MetricType.Rep,
      origin: 'runtime',
      action: 'suppress',
    });
...
// ── 5. Legacy seam characterisation ──────────────────────
