...
const CardTimerPill: React.FC<{ 
    ...
    const displayTime = Math.max(0, format === 'countdown' && duration ? duration - elapsed : elapsed);
...
export const RefinedTimerDisplay: React.FC<RefinedTimerDisplayProps> = ({
    ...
    const progress = useMemo(() => {
        ...
        if (effectiveTimerState.format === 'countdown' && effectiveTimerState.duration) {
    ...
    // Pulse animation for non-countdown timers (countup or undefined/infinite)
    const isPulsing = isRunning && (
        !effectiveTimerState?.duration || 
        effectiveTimerState?.format === 'countup'
    );
