...
import { RoundsMetricInheritance, ProgressiveResistanceInheritance, PercentageProgressionInheritance, TimeBasedInheritance } from "./ExampleMetricInheritance";
...
describe("Metric Inheritance System - Complete Test Suite", () => {
    ...
    describe("TimeBasedInheritance - Endurance Training Scenarios", () => {
        test("Story: Marathon training pace adjustment", () => {
            ...
            // Add 30 seconds for easy pace
            const easyPace = new TimeBasedInheritance(30);
            const result = composer.compose([easyPace]);
            
            // 9600 + 30 = 9630 seconds
            expect(result[0].values[1].value).toBe(9630);
        });
    ...
    describe("Complex Inheritance Chaining - Real Workout Scenarios", () => {
        ...
        test("Story: Powerlifting meet prep with percentage and progression", () => {
            ...
            const warmup = new ProgressiveResistanceInheritance(-50, 2); // Back off 50lbs
            
            const result = composer.compose([opener, warmup]);
            
            // 500 × 0.90 = 450, then 450 + (-50 × (2-1)) = 450 - 50 = 400 lbs
        ...
        test("Story: Triathlon training with multiple inheritance types", () => {
            ...
            // Swimming: 1000m (unchanged by rounds), then × 1.1 = 1100m
            expect(result[0].values[0].value).toBe(1100);
            // Swimming time: 1200s (unchanged by rounds), + 60s = 1260s, × 1.1 = 1386s
            expect(result[0].values[1].value).toBe(1386);
        });
    });
    ...
    describe("Edge Cases and Error Handling", () => {
        ...
        test("should handle floating point precision", () => {
            ...
            // 100.5 × 0.3333 = 33.498... which rounds to 33
            expect(result[0].values[0].value).toBe(33);
        });
    });
