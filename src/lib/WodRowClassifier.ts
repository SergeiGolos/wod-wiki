...
export class WodRowClassifier {
  static isNotification(block: StatementBlock): boolean {
    return block?.type === 'notification';
  }

  static isHeader(block: StatementBlock): boolean {
    return block?.level === '#' || block?.type === 'header';
  }

  static isParagraph(block: StatementBlock): boolean {
    return block?.type === 'paragraph';
  }

  static getExerciseParts(block: StatementBlock): string[] {
    const parts: string[] = [];
