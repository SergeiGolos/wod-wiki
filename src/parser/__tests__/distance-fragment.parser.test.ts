...
describe('Distance fragment parsing', () => {
  it('parses distance with number (400m) with amount, units, and Defined state', () => {
    ...
    expect(distance.collectionState).toBe(FragmentCollectionState.Defined);
  });
  ...
  it('parses collectible distance (?m) with undefined amount and UserCollected state', () => {
    const script = parse('?m');
    const distance = script.statements[0].fragments.find(f => f.fragmentType === FragmentType.Distance) as DistanceFragment;
    ...
    expect(distance.collectionState).toBe(FragmentCollectionState.UserCollected);
  });
