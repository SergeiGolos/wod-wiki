...
describe('CastSessionManager.registerRuntime / unregisterRuntime', () => {
    let transport: MockTransport;
    let registry: CountingRegistry;
    let mgr: CastSessionManager;
    let eventProvider: MockEventProvider;
    let clockSync: MockClockSync;
    let fakeRuntime: { stackCallbacks: any[]; outputCallbacks: any[]; subscribeToStack: any; subscribeToOutput: any };
    ...
    beforeEach(() => {
        ...
        eventProvider = built.eventProvider;
        clockSync = built.clockSync;
        mgr = new CastSessionManager(built.deps);
        fakeRuntime = {
            stackCallbacks: [] as any[],
            outputCallbacks: [] as any[],
            subscribeToStack: (cb: any) => { fakeRuntime.stackCallbacks.push(cb); return () => {}; },
            subscribeToOutput: (cb: any) => { fakeRuntime.outputCallbacks.push(cb); return () => {}; },
        };
    });

    it('throws when no session is active', () => {
        expect(() => mgr.registerRuntime(fakeRuntime as any)).toThrow('no active session');
    });

    it('adds a subscription, wires stack and output listeners, returns the id', () => {
        mgr.connect(transport, registry);
        const returnedId = mgr.registerRuntime(fakeRuntime as any, 'inline-timer');
        expect(returnedId).toBe('inline-timer');
        expect(fakeRuntime.stackCallbacks.length).toBe(1);
        expect(fakeRuntime.outputCallbacks.length).toBe(1);
    });

    it('unregisterRuntime disposes the subscription', () => {
        mgr.connect(transport, registry);
        mgr.registerRuntime(fakeRuntime as any, 'inline-timer');
        const sub = (mgr as any).extraSubscriptions.get('inline-timer')?.subscription as MockSubscription;
        expect(sub.disposeCalls).toBe(0);
        mgr.unregisterRuntime('inline-timer');
        expect(sub.disposeCalls).toBe(1);
    });

    it('unregisterRuntime is idempotent on missing ids', () => {
        mgr.connect(transport, registry);
        expect(() => mgr.unregisterRuntime('nope')).not.toThrow();
    });

    it('dispose() tears down all extra subscriptions', () => {
        mgr.connect(transport, registry);
        mgr.registerRuntime(fakeRuntime as any, 'inline-1');
        mgr.registerRuntime(fakeRuntime as any, 'inline-2');
        const sub1 = (mgr as any).extraSubscriptions.get('inline-1')?.subscription as MockSubscription;
        const sub2 = (mgr as any).extraSubscriptions.get('inline-2')?.subscription as MockSubscription;
        mgr.dispose();
        expect(sub1.disposeCalls).toBe(1);
        expect(sub2.disposeCalls).toBe(1);
    });
});
