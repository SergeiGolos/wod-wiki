/**
 * anchor option tests (#857) — `FindOptions.anchor: 'latest-activity'`
 * resolves `last <n>d|w` windows against the newest createdAt in the
 * scope-selected set instead of wall-clock now, keeping windows meaningful
 * on snapshot/static corpora. Default behavior (no option) is unchanged:
 * wall-clock now. An explicit `range` still wins over everything.
 */
...
const TS_BASE = 1_700_000_000_000; // 2023-11 — months before any test run's now
...
// Newest = T0; recent is 5d before (inside 2w of T0); old is 30d before (outside).
const T0 = TS_BASE;
...
describe("anchor: 'latest-activity' — runFind", () => {
  it('windows against the newest entry, keeping recent-but-old-in-wall-clock notes', async () => {
    ...
    // Wall-clock now would drop all three (fixtures are from 2023-11).
