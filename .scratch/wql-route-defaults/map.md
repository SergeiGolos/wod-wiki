...
- **Naming guard**: do not call the user config a "profile" — that name is taken by `StreamProfile`. See `playground/CONTEXT.md`.
...
- Build tickets, graduating once the decision tickets close: settings sub-page implementation; wiring the resolution seam into `QueriableStreamView`/`App` bare-landing seeding; palette wiring; whether the palette's dual `find:block` secondary dispatch follows its configured default (hangs off "Palette default override").
- Test strategy for resolution precedence (config → system default → URL `?q=` wins over both).
