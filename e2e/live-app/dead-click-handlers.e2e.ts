...
test.describe('Live app dead click handler regressions', () => {
  test('collection workout cards navigate to the workout editor', async ({ page }) => {
    ...
    await page.screenshot({ path: 'e2e/screenshots/dead-click-collections-workout.png', fullPage: true });
  });

  test('journal plan-a-workout slots open the selected date editor', async ({ page }) => {
    ...
    await page.screenshot({ path: 'e2e/screenshots/dead-click-journal-plan-slot.png', fullPage: true });
  });
});
